...
**`ExerciseType` is the central config hub** (`Form/Form/Features/FormAnalysis/FormAnalyzer.swift`). Each case carries its own `repDetectionJoint` (which 3 joints define the tracked angle), `repThresholds` (low/high angle gates), `symbolName` (SF Symbol for selection UI), and `cameraSetup` (per-exercise phone-placement guidance — drives the "positioning" tip shown before a set, framed to match the joints each analyzer needs). Adding an exercise = add a case here (fill in all of those), add a `FormAnalyzing` struct, and register it in `makeAnalyzer(for:)` — `WorkoutView` and `RepCounter` need no changes (Strategy + Factory).
...
**User preferences** live in `SettingsStore` (`Form/Form/Persistence/`), a `UserDefaults`-backed `ObservableObject` that writes through on `didSet`. It holds `VoicePreferences` (`Form/Form/Models/` — a pure `Codable` value type that clamps pitch/rate to `AVSpeechUtterance`'s ranges; the voice is a `String` identifier, never an `AVSpeechSynthesisVoice`, to stay testable) and `ExerciseSelectionStyle` (A/B toggle: dedicated `ExerciseSelectionView` vs. inline HUD picker — *both* ship intentionally). `WorkoutView` pushes voice prefs into `AudioCueEngine.apply(_:)` on appear and `onChange`. The backing `UserDefaults` is injectable so `SettingsTests` can assert persistence against an isolated suite. Because the Xcode project adds source as **groups** (not synced folders), any new `.swift` file must be manually added to the target — `swift test` will pass without it, but the device build will fail with "Cannot find type … in scope".
